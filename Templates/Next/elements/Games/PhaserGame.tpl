...
{% set gameName = element.values.name|default(element.unique_id) | friendly | capitalize %}
{% set gamePath = '/game/Game_' ~ gameName ~ '.tsx' %}
{% set scenes = [] %}
...
{% set forget %}
  ...
  {% endblock %}
  {{ addExtraFile('src' ~ gamePath, block("baseGame")) }}
{% endset %}
{% set bpr %}
