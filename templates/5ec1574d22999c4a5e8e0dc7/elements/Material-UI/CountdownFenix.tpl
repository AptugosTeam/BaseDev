...
const Timer = ({ days, hours, minutes, seconds, paragraphColor }) => {
  return (
    <div 
    ...
    {% endif %} 
    >
    <p style={ { color:paragraphColor} }>{zeroPad(hours + (days*24))}:{zeroPad(minutes)}:{zeroPad(seconds)}</p>
    </div>
  )
}
{% endset %}
...
<Countdown
  {% if element.values.dateToCount %}
    date={ {{ element.values.dateToCount|raw }} }
  {% else %}
  ...
  {% endif %}
  {% if element.values.oncomplete %}onComplete={(e) => {{element.values.oncomplete}} }{% endif %}
  renderer={(props) => {
    ...
    {% else %}
    let theColor = (props.hours + (props.days * 24)) < 24 ? "#F08891" : "#262626"
    if (props.hours + props.days * 24 < 24) {indicatorStyles = { ... indicatorStyles, background: '#F08891'} }
      return (
        <Timer
          days={props.days}
          hours={props.hours}
