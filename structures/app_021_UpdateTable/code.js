try {
  ...
  const parseFieldPayload = () => {
    if (typeof Parameters.FieldPayload === 'object') return Parameters.FieldPayload
  ...
  const applyToTable = (tableRef) => {
    ...
    return {
      nameChanged: oldName !== table.name,
      singleNameChanged: oldSingleName !== table.singleName
    }
  }
  ...
  if (Array.isArray(Parameters.Tables)) {
    tableRefs = Parameters.Tables
  } else if (Parameters.Table) {
    tableRefs = [Parameters.Table]
  } else {
    return { error: 'Missing parameter: Table or Tables' }
  }

  const results = []

  for (const ref of tableRefs) {
    const result = applyToTable(ref)
